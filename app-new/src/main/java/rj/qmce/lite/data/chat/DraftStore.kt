...
object DraftStore {
    ...
    fun save(context: Context, peerUid: String, chatType: Int, text: String) {
        ...
        if (text.isBlank()) {
            ...
            context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit().putString(k, text).apply()
        }
    }

    fun load(context: Context, peerUid: String, chatType: Int): String =
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).getString(key(peerUid, chatType), "") ?: ""

    fun clear(context: Context, peerUid: String, chatType: Int) {
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit().remove(key(peerUid, chatType)).apply()
    }
